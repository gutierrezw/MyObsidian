...
| 3.3 | 2026-07-12 | Agregada columna `tablas_afectadas` a `reportes_historial` (ver [[design-report-center]]) para estadísticas de qué tabla concentra hallazgos. Para `full_scan`, `schemaHealth.js` ahora también trae `query_sample_text` del catálogo (muestra real ejecutada, no el digest normalizado de 80 caracteres) — es la única forma de saber qué tabla(s) toca un full scan, porque `events_statements_summary_by_digest` no tiene columna de tabla (confirmado en el catálogo real, ni siquiera la vista `sys.statements_with_full_table_scans` la tiene). `referencia` se sigue armando con `digest_text` (estable entre corridas) para no romper la serie histórica |
| 3.4 | 2026-07-12 | `ReportManager.registrar()` pasa a upsert (ver [[design-report-center]] v2.3) — cada corrida semanal actualiza el caso activo en vez de insertar fila nueva. Probado end-to-end contra `bdinv` real: dos corridas seguidas del trigger dejaron `total_filas = referencias_unicas = 39` |
