...
| 🧪 78 | **Infraestructura/Performance** | **Desface performa_inversion Crypto (2026-08-05→08-13)** — `diaria_performance` generaba valores incorrectos para Crypto. **Root cause encontrado:** `detalle_book()` usaba Yahoo Finance (`vehiculo="download"`) para Crypto en lugar de Binance klines. Yahoo Finance no reconoce tickers USDT (BTCUSDT, ADAUSDT, etc). **Fix aplicado (commits fd5282e, 6828c55):** (1) detectar `categoria='Crypto'`; (2) usar `vehiculo="Crypto"` → `get_klines_info()` (Binance); (3) pasar ticket sin convertir. **Estado:** En observación — reiniciada AppOO, esperando ejecución normal diaria_book_performance próximas 24h. | Media |
