...
| 79 | **UI/Style Guide** | **Migrar resto de la app a `Flat.TButton` + Segoe UI 9 + Entry con bg/fg explícitos** — hoy solo migradas Editar Plan, Editar Vehículo y el botón Cancel de Diversificación (ver `spec-style-guide.md`). Migrar el resto (Screener, Consenso, popups de órdenes, Setup general, etc.) de a una, cuando se toquen por otro motivo — no es un sprint de refactor dedicado. | Baja |
