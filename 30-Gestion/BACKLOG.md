...
| 70 | **Infraestructura/Salud** | Report Center — motor genérico `reportes_historial` + `ReportManager` + `GET /reports/:tipo` + Cloudflare Access. Primer consumidor: `schema_health` (extiende Fase 1 del ítem #69, no la duplica). Diseño en `design-report-center.md`/`design-schema-monitor.md`. **Backend implementado y Task Scheduler migrado 2026-07-12**: tabla + `lib/ReportManager.js` (upsert por caso activo, no duplica filas por corrida) + `routes/reports.js` + columna `tablas_afectadas` (vía `QUERY_SAMPLE_TEXT`) + "Reporte Semanal MySQL" repuntada a `scripts/trigger-schema-health.ps1`, todo probado end-to-end contra `bdinv` real. Falta: (a) página `GET /reports/:tipo` con UI (hoy devuelve JSON crudo), (b) Cloudflare Access sobre `/reports/*`, (c) archivar `mysql-weekly-report/` tras validar 2-3 semanas, (d) implementar umbral de severidad 10M filas para `full_scan` (documentado, no codeado). | Media |
