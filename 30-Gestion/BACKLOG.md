...
| 84 | **Stock/Preservation** | **El EXIT cancela en el broker aunque Preservation esté en DRY-RUN** — `_preservation_run_vehiculo()` (`Class_AgentManager.py:1021-1050`) llama a `DataHub.preservation_cancel_order()` en la rama EXIT sin ninguna guarda de simulación; `is_live` se calcula recién en la línea 1171, muy por debajo. Un DRY-RUN que debería no tocar nada cancelaría una orden real en IB. **Hoy no dispara**: la rama EXIT necesita un símbolo con `order_id` persistido en `deploy/tmp/preservation_state.json`, y ese archivo hoy solo tiene `_last_run_Stock` / `_last_run_Crypto`. **Se abre apenas Preservation persista un STOP** — es decir, en la primera corrida que emita de verdad. **Anotado para pensarlo el 2026-08-31, no arreglado**: el fix (mover el cálculo de `is_live` arriba de la rama EXIT, o guardar la cancelación) es chico, pero toca el orden de un método que se está observando en producción y no se quiso mezclar con el cambio de ventana ni con el contador. Retomar antes de sacar el DRY-RUN. Ver [[design-preservation]] § "Lo que falta antes de sacar el DRY-RUN". **Bloquea la salida de DRY-RUN de Preservation** — hilo heredado del ítem 76 al cerrarse el 2026-09-09: mientras el EXIT cancele en el broker sin guarda de simulación, `_preservation_dry_run = False` no es una decisión que se pueda tomar. | **Alta** |
| 85 | **Venta/Trazabilidad** | **No hay puente entre una recomendación y la orden que produjo** — `symbol_decision_history.order_trader_id` está en NULL en el 100% de las filas (Preservation y GainsCapture lo pasan literalmente como `None`), y `order_trader.json_audit_log` está vacío en las 50 órdenes de agosto pese a que [[MEMORY]] lo describe como "el HUB de historial de decisiones". Con las dos puntas nulas **no existe forma de unir una decisión con su orden**: se puede ver qué recomendó Claude y se puede ver qué se ejecutó, pero no que lo segundo salga de lo primero. Consecuencia práctica: no se puede medir la tasa de acierto de los módulos de venta, que es justo el dato que pide #83 para calibrar el modo autónomo. **Anotado para pensarlo el 2026-08-31.** El contador `veces` (2026-08-31) redujo el ruido de la tabla — 64 filas a 9 — pero no toca este puente. Evidencia y caso testigo (BTG vendido a mano el 25/08 mientras GainsCapture recomendaba vender) en [[design-gains-capture]] § "La traza cuenta repeticiones en vez de repetirse". **A decidir antes de implementar:** si el puente se escribe al emitir (el agente ya tiene el `id` de `order_trader`) o al confirmar en `Agente_SyncOrders`; y si `json_audit_log` sigue teniendo razón de ser una vez que exista `order_trader_id`, o queda como duplicado. | Media |
| 86 | **IA/Sentimiento** | **El sentimiento se carga para una sola cuenta y el CSV de venta trae tres** — `predecir_oportunidades()` (`Class_IA_modelos.py:494` y `:952`) llama `MarketScreen().load_sentiment_features(account)` con **una** cuenta; los dos paneles de `Class_SystemStatus.py` (`:3164`, `:3731`) hacen lo mismo con `chatbot.account`. El CSV de venta mezcla `U4214563`, `BBVA0001` y `SANT0001`, así que los símbolos de las otras dos cuentas predicen con las features de sentimiento rellenadas en 0.0. **Síntoma medido 2026-09-03:** los FCI del panel Sell (SUPERFONDO, FBA ACCIONES, SUPERGESTIÓN) salen todos entre 0.43 y 0.47 — confianza aplastada al centro, sin capacidad de discriminar entre ellos. El WARNING de `ClassMoodeloIA: predecir_modelo(): features ausentes rellenadas con 0.0` es el que lo delata. **Ruta:** cargar el sentimiento por las cuentas presentes en el df (agrupar por `account` y hacer el merge por cuenta), no por una sola. Sin esto cualquier calibración de umbral sobre FCI mide ruido. | **Alta** |
| 87 | **IA/Dataset** | **La etiqueta del dataset dice "no la ejecuté", no "era mala"** — Métricas al 2026-09-03: **Sell** 121 muestras, accuracy 79%±6; las 71 *rechazadas* promedian ROI 37,5% y $65 de profit contra 43,3% y $102 de las 50 aprobadas — la separación es chica porque `rec=-1` marca lo que el usuario no ejecutó, no una operación perdedora. Ese es el techo del 79% y no lo mueve reentrenar. **Buy** está peor: 84 muestras, accuracy 61%±8, contra un baseline de clase mayoritaria de 51% (43 de 84) — el modelo aporta ~10 puntos con ±8 de desvío, por eso sigue en modo **Etiquetando**. **Nota operativa:** en ambos paneles "Muestras para entrenar" == "Total con decisión" (84 y 121), o sea el modelo ya consumió todo el dataset y reentrenar hoy devuelve el mismo modelo; el entrenamiento es manual (`run_entraminetoSell` / `run_entrenamientoBuy`, botón **Entrenar**), no hay agente. **Ruta:** definir una etiqueta con resultado real — cruzar contra `booktrading` el ROI que la operación habría dado — en vez de la decisión del usuario. Se apoya en la herramienta del ítem 83. | Media |
| 88 | **Venta/Trazabilidad** | **`json_detalle.capas` sólo existe para SELL** — el rastro de las 7 capas se implementó únicamente en el camino de venta (ver v5.2). BUY tiene otros nombres de capa y hoy corre con la capa 3 en cortocircuito y la capa 6 salteada por `modo_etiquetado_buy`, así que replicarlo tal cual no aplica. **Además queda abierto, del mismo trabajo:** (1) una fila rechazada en capa 3 sólo deja rastro si el símbolo **ya** tiene registro en `oportunidadesbuysell` — las filas nacen en capa 4, un símbolo que nunca llegó ahí no tiene dónde escribir, y ese caso pertenece a `symbol_decision_history` (necesita diseño de dedup, ver ítem 85); (2) el orden real del código no es 6→7 — el gate de calendario (numerado 7 por convención del usuario) corre **antes** de `Agente_message_Manager_sell` (6), así que con el mercado cerrado el JSON muestra `7:false` sin `6`. Se dejó la numeración del usuario a propósito: es la que usa para leer el flujo. | Baja |
...
### v5.5 — 2026-09-09
**GainsCapture y Preservation salen de observación — ítems 53 y 76 CERRADOS:**
- ✅ **ítem 🧪 53 — `Agente_GainsCapture`.** Desbloqueado el 2026-08-29 (H5 cerrado con el gate cruzado). El único punto que quedaba abierto era calibrar `min_ganancia`, resuelto hoy: Stock baja de **200 a 150** en `sesion.parameters`. Medido con `AppTest/run_booktrading_roi.py` sobre la ventana vigente (12m, 61 órdenes): la cobertura pasa de **4 órdenes (6,6%) a 10 (16,4%)** — B ×2, HASI ×2, PBR ×2, de 157 a 196 USD y ROI 33–85%, ninguna marginal. El valor cargó en vivo a las **06:43:18** (`config cargada | min_roi=20% | min_gan=150.0 | modo=SUPERVISADO`). Crypto queda en 300, sin tocar.
  ⚠️ **Se cierra sin una sola orden emitida, y eso queda dicho.** GainsCapture no produjo ninguna venta en 12 meses. La corrida de las 06:43, ya con 150, sigue en cero: de **13 candidatos** `categoriaActivo='N'`, **10 mueren en `min_roi` y 3 en `min_ganancia`**. Lo que se cierra es la implementación, no la prueba de que el mandato dispare. **El gate que hoy manda es `min_roi=20%`**, no la ganancia — si el agente sigue mudo, ese es el parámetro a mirar, y el hilo vive en el ítem 83.
  **Doc:** `20-Proyecto/design-gains-capture.md` § "Calibrado 2026-09-09" — `e46d6d2` (vault).
- ✅ **ítem 🧪 76 — Preservation Fase 1 mejorada.** Los cinco puntos del ítem están implementados y los cinco hallazgos que la revisión independiente abrió sobre él quedaron cerrados el 2026-08-27 (H3 ratchet, H4 cancel+send, H6 Crypto en DRY-RUN, H7 `PRECIO_MINIMO` eliminado, H8 fallback SMA20). Se suma hoy el reorden del gate del techo, que evita el contexto de Claude en los símbolos que igual se descartan sin cambiar ninguna decisión — `be8cd8b` (MyPython), `f858d54` (vault).
  ⚠️ **Se cierra con el módulo en DRY-RUN y con el reorden sin correr una sola vez.** `_preservation_dry_run = True` para los dos vehículos (`Class_AgentManager.py:67`) y el log todavía no tiene ninguna línea `→ SKIP (sin consultar a Claude)`: la app corre el código anterior y necesita reinicio con `__pycache__` limpio. **El hilo de salir de DRY-RUN no muere con este ítem** — queda anclado al ítem 84, que es su bloqueante.
- **Qué NO se cierra con esto:** el ítem **83** (calibración por ventana — sigue abierto para Crypto, para Oportunidades y para el `min_roi` de GainsCapture) y el ítem **84** (el EXIT cancela en el broker aunque Preservation simule).
- **Estado:** CERRADOS por implementación completa y sin bloqueantes. La validación en vivo de los dos módulos continúa bajo los ítems 83 y 84, no bajo estos.

### v5.4 — 2026-09-06
**Los importes del panel se leen de un vistazo — `mask_numero()` reutilizado:**
- ✅ **El dGyP y los importes del header/grid pasan a escala K/M/B.** Se mostraban completos (`dGyP: -36519`, `Valor liq. 12617179.29`) y en ARS no se leen de un vistazo. **No se creó un formateador nuevo:** `mask_numero()` (`Modulos_Utilitarios.py`) ya escalaba a K/M/B/T para el Screener y se extendió con `decimales`, `ancho` y `base`. Sin `base` sigue devolviendo `str(numero)` — es de lo que depende `Class_Screener.py:888`, así que el uso existente no cambia. Un valor no numérico vuelve intacto: el header mezcla importes con `Conexión` y `%Mrg/Risk`.
- ⚠️ **El mask se aplica al texto que se pinta, nunca al dato — lo advirtió el usuario y evitó el bug.** Los importes del panel se releen como número y se suman: `self.resumen` pasa por `float()` en Cash y Dividendos (`Class_customer.py:2150`, `:4643`, `:4703`, `:6721`), `create_styles()` decide el color contra `row[idx]` numérico y la fila de totales se calcula sobre los números. Guardar el string enmascarado rompía esas tres cuentas **en silencio**, porque el `try/except` de cada método se traga el `ValueError`. Por eso `set_header_panel()` sigue guardando `"{:>11.2f}"` y el mask vive en el `.config(text=...)` y en el `values=` del Treeview. `DashMain.py:1281` también escribe Cash con ese formato. Del Treeview solo se re-lee la columna `ticket`.
...
- ✅ **Preservation Crypto no podía evaluar ningún símbolo — doble conversión de ticket.** BNBUSDT calificaba (`ROI=18.4% ≥ 18% → evaluando`) y moría en el paso siguiente (`datos insuficientes (0 rows, CacheHut + yfinance) → SKIP`), **9 corridas seguidas** cada 2h entre el 2026-09-03 11:37 y el 2026-09-04 03:37. `get_yfinance(vehiculo="Crypto")` **espera el símbolo Binance**: convierte internamente para `yf.Ticker` y pasa el `ticket` crudo a `get_klines_info()`. `preservation_get_atr()` y `preservation_get_sma()` (`Class_customer.py:1082` y `:1103`) lo pre-convertían con `convierte_ticket_crypto()`, así que Binance recibía `BNB-USD` y devolvía 0 filas. **El fallo es silencioso** porque la conversión es idempotente sobre `BNB-USD` y nada levanta excepción — solo el `return None, "datos insuficientes"` del helper, que el loop trata como SKIP normal. Afectaba a los **dos** helpers, o sea ATR y SMA20: sin ellos no hay stop y **ningún símbolo Crypto llegaba a evaluarse**. Corregido pasando `symbol` directo. **Stock nunca estuvo afectado.** No se detectó antes porque Crypto entró al loop recién el 2026-08-31 y en DRY-RUN. **Encontrado por lectura de código, no reproducido** — falta verlo en la próxima revisión Crypto en vivo.
- ✅ **El log de venta pasa de ~288 líneas/día a ~2 por reinicio más una por cambio real.** De 711 líneas del archivo, **455 son de los dos días post-deploy** de la apertura de corrida (`eef4873`) y ~30 son señal. GainsCapture escribía 3 líneas por corrida por vehículo cada 30 min y las tres eran idénticas entre corridas. Dos cambios: (1) la config (`min_roi`, `min_gan`, `modo`) se loguea **al cargarla y cuando cambia**, igual que la línea `config cargada` que Preservation ya tenía, en vez de repetirse en cada corrida; (2) el par posiciones + CIERRE se emite **solo cuando cambia el desglose**, con `repetidas=N` diciendo cuántas corridas mudas hubo antes — mismo criterio que `symbol_decision_history.veces`. El estado vive en memoria del proceso: tras un reinicio la primera corrida vuelve a emitir todo, que es lo correcto porque un reinicio es información. **No se revierte la apertura** — sigue haciendo falta distinguir "corrió y no había candidatos" de "no corrió"; lo que cambia es cuándo habla.
- ⚠️ **Falsa alarma descartada, queda anotada.** El conteo global de aperturas contra cierres daba desparejo (GainsCapture 99/71, Preservation Crypto 48/17) y parecía un `return` mudo a mitad de corrida. No lo es: el CIERRE lo agregó `eef4873` (2026-09-02 18:45) y todas las aperturas huérfanas son anteriores. **Desde el 2026-09-03 el pareo es exacto** (2/2, 15/15, 63/63, 63/63). Anotado para no volver a investigarlo.
- **Señal que sí dejó el log:** PBR (Stock) subió de ROI 12,2% a 20,6% entre el 08-31 y el 09-03 sin gatillar nunca — siempre `protege N < gainInversion 70.00 → SKIP`, con máximo en `protege 65.67` el 09-02 12:36, corto por **4,33**. Entre el 09-02 y el 09-03 la posición pasó de `qty` 22 a 14 (venta manual). Es el dato que pide el ítem 83 para calibrar.
- **Doc:** `20-Proyecto/design-preservation.md` § "Preservation Crypto no podía evaluar ningún símbolo" y `20-Proyecto/design-gains-capture.md` § "La apertura se emitía en cada corrida".
- **Estado:** comiteado el 2026-09-06 — `fb53756` (MyPython). **Falta verificarlo en vivo:** la revisión Crypto corre cada 2h y tiene que emitir `Preservation(Crypto/BNBUSDT): ... ATR=... sma20=...` en vez de `datos insuficientes (0 rows, CacheHut + yfinance) → SKIP`. Si vuelve `datos insuficientes`, la causa es otra y la doble conversión no era el problema.

### v5.2 — 2026-09-03
**Camino único de predicción y trazabilidad de las 7 capas de venta — SIN COMITEAR (validado en vivo):**
- ✅ **El panel y el agente dejan de dar números distintos** — el panel Sell/Buy IA reconstruía la predicción por su cuenta y llegaba a otro número que el agente sobre la misma fila. Caso testigo: **PBR 33% — panel 0.9449 contra agente 0.5705**; en BUY la divergencia además invertía el resultado (PFLT 0.470 mostrado contra 0.694 real, 3 COMPRAR en pantalla contra 5 reales). Ahora los dos caminos entran por `predecir_oportunidades(df, account, columnas_map)` (`Class_IA_modelos.py`), que fija la secuencia `hash_id → rename → aplanar → sentimiento → modelo → merge por hash_id`. Se agregó un candado: `ClassMoodeloIA` emite **WARNING** cuando `predecir_modelo` rellena features ausentes con 0.0 — es el que destapó el ítem 86. El logger ya está en WARNING por defecto (`Class_debugging.py:338`); si el panel Debugging lo subió a ERROR en una sesión previa, `tmp/logger_levels.json` lo reaplica al arrancar y el mensaje no se escribe.
- ✅ **Traza de las 7 capas en `oportunidadesbuysell.json_detalle.capas`** — cada capa del camino de venta se marca **donde realmente decide** (no hay un reconstructor central), en un objeto `{"<capa>": {"ok": bool, "dato": "<motivo o valor medido>"}}`. Las claves son los nombres reales de los métodos, a pedido del usuario: `1_csv_OptionSales_write`, `2_Agente_ManagerSell`, `3_evaluar_oportunidades_sell_con_IA`, `4_oportunity_handler_sell`, `5_gate_menu`, `6_Agente_message_Manager_sell`, `7_opportunity_handler_message_sell`. Método nuevo `actualizar_capas()` (`Modulos_Mysql.py`) con `JSON_SET` sobre la columna `json` — **no toca `timestamp`**: esa columna dice cuándo corrió el pipeline completo y es diagnóstico cargado (una fila de venta congelada a las 18:14 mientras las compras marcaban 20:12 fue lo que probó que el pipeline de venta se había detenido); anotar una capa no es una corrida.
  **Defecto encontrado y corregido en la misma sesión, a raíz de que el usuario pidió sustentar el volumen de escrituras:** `Agente_ManagerSell` **no tiene `@wait_rate`** — corre dentro de `agentesIA()` con `time.sleep(15)` todo el día → **5.760 ciclos/día** × 23 filas del CSV = **~132.000 UPDATE/día** si se escribe en cada pasada. El candado `_capas_ultimo` firmaba `json.dumps(capas)` **incluyendo el `dato`**, y el dato de la capa 1 lleva ROI y ganancia vivos que se mueven con cada tick: la firma cambiaba casi siempre y el candado no frenaba nada — exactamente las escrituras que decía evitar. Corregido: la firma toma **sólo los `ok`**. **Consecuencia aceptada:** el `dato` guardado es una foto del momento en que el veredicto cambió por última vez, no un valor vivo; para valores vivos están `roi` y `profit` en el mismo `json_detalle`, que sí se refrescan en cada corrida.
  **Verificado en vivo tras reinicio (2026-09-03):** la fila 658 (PBR 33%) escribió capas 1–3 con `3_evaluar_oportunidades_sell_con_IA` en `false` (`confianza 0.57 vs umbral 0.65`) y **dejó el `timestamp` intacto** en 02:14:40. Las capas 4–7 no aparecen porque la corrida se corta en la 3, que es el comportamiento esperado. Las filas anteriores al cambio quedan sin capas — no se rellenan hacia atrás.
- **Doc:** `20-Proyecto/ref-oportunidades.md` v2.3 — secciones nuevas **§8 Camino único de predicción** y **§9 Trazabilidad — json_detalle.capas**.
- **Estado:** comiteado y pusheado el 2026-09-03 — `0cf9c09` (MyPython) y `4e5f294` (vault). *(El título decía "SIN COMITEAR": era cierto al escribirlo, quedó superado ese mismo día.)*

### v5.1 — 2026-08-30
**TWR real, deriva del reloj, ganancias realizadas Crypto y monitoreo de schema — CERRADOS:**
- ✅ ítem 74 — **TWR real para cartera Stock**: la rama `tipo in ("Stock", "Crypto")` de `performa_asset()` (`Modulos_Comunes.py`) ya no usa el ratio simple `performa / raiz - 1`; calcula TWR diario `value / (value.shift(1) + new_capital) - 1` y lo compone, reutilizando `select_bbva_ars_cashflows(categoria=, cuentas=)` — el mismo método de BBVA.ARS parametrizado, no uno nuevo. Umbral de anulación de saltos 0.50 (contra 0.30 de FCI) porque Stock/Crypto son más volátiles. **Dos defectos encontrados al cerrarlo, ambos corregidos en la misma sesión:** (1) el cashflow se calculaba con `basico` — el promedio móvil de la posición — en vez de `preciotrans`, el precio real de la operación, inflando el denominador y empujando el acumulado hacia −100% (Stock: 93.068 de flujo contra 51.222 reales; commit `28c3ee4`); (2) las ventanas del gráfico recortaban la serie acumulada sin rebasear, así que 1M/3M/6M/1Y/5Y mostraban el mismo % final y solo cambiaba el zoom — ahora se recomponen contra el primer día de la ventana, `(1+c)/(1+c[0])-1` (commit `52b414c`). El mismo error, en su variante de **resta de acumulados**, vivía también en `crear_grafico_vs_indice()` de `Class_Analisis.py` y afectaba cartera, índice y Alpha (commit `924ad94`). Resultado: Stock −92,76% → **+1,64%**, Crypto −98,87% → **−93,67%**, BBVA.ARS −54,54% → **−24,45%**. Detalle en `10-Memoria/sesion-2026-08-30-twr-y-rebase.md`.
- ✅ ítem 🧪 84 — **Deriva del reloj**: se cierra por evidencia de las dos capas sosteniéndose, no porque se haya encontrado la causa física. `time_watchdog.csv` acumula 427 mediciones desde 2026-08-27: **426 `OK`** con offset típico bajo 2 ms, y **un solo `RESYNC`** el 2026-08-30 05:38 con −0,58 s — fuera de la grilla de 10 minutos, o sea un arranque/reanudación, no deriva acumulada; el watchdog lo corrigó a 1,5 ms en el mismo tick. Eso es exactamente lo que la red de seguridad existía para hacer. **Lo que queda como deuda conocida:** los 690 ppm siguen intactos y sin diagnosticar (pila CMOS, C-states, HPET). Está compensada, no resuelta — si W32Time o el watchdog se caen, el problema vuelve entero. Reabrir si aparecen `RESYNC` recurrentes en el CSV.
- ✅ ítem 82 — **`diaria_performance` y las ganancias realizadas de Crypto/BotCrypto**: se cierra con el alcance que el propio ítem declaraba — *solo diagnosticar*. Causa confirmada en código: `acumula_igual_date()` (`Modulos_Comunes.py`) escribe `gprealizadas` únicamente cuando la fecha del `booktrading` existe en los datos de mercado del símbolo; si ese día no vino de Yahoo, la venta no tiene fila donde aterrizar. De ahí las 82 combinaciones sin fila y las 78 con `gyp_dia` distinto.
  **Relación con el 74, que es lo que permite cerrarlo:** este ítem nació persiguiendo el rendimiento absurdo de Crypto, y hoy quedó establecido que ese −98,87% era la fórmula de cashflow (`basico` en vez de `preciotrans`), **no** este hueco de datos — el ítem pierde su rol de explicación del síntoma. Corregida la fórmula, Crypto da −93,67% con los mismos datos.
  **Lo que el 74 sí dejó acotado:** `crea_dataframe_diaria()` compone `value Portafolio = value + dividends + gyp_dia`, o sea el hueco **sí** toca la curva TWR, pero por un monto medido y chico — +$186,95 Crypto / −$78,25 BotCrypto sobre una cartera de ~$7.000. No es lo que movía el número.
  **Deuda conocida, no reabierta:** el fix de `detalle_book` (que una descarga vacía deje traza en vez de perder el día) y el reproceso de las tres capas encadenadas siguen sin hacer. Reabrir con alcance de *fix*, no de diagnóstico, cuando se decida tocar el histórico — `IPerformance.purgar_desde()` y `AppTest/run_purgar_performance_desde.py` ya existen para esa fase.
- ✅ ítem 69 — **Tools MCP schema MySQL Fase 1**: verificado en `MyNode/server-api/routes/mcp.js` (líneas 258–290). `get_schema_health` y `get_slow_queries` existen, son read-only y ambos loguean por `audit()` en `mcp_audit.jsonl`. `run_schema_fix` **no** está implementado, que es exactamente lo que el ítem resolvía: queda pausado hasta que aparezca un caso de uso real que lo justifique.
- ✅ ítem 70 — **Report Center**: backend completo y en producción. `lib/ReportManager.js` + `lib/reportPage.js` + `routes/reports.js` con las 7 rutas (`GET /:tipo`, `GET /:tipo/historico`, `POST /:tipo/run`, `/resolver`, `/proponer`, `/descartar`, `/schema_health/reset-stats`); `no_reproducido`, `pesoRegistro()` y `marcarPropuesto` presentes. **Evidencia de operación:** `reportes_historial` tiene 52 hallazgos de `schema_health`, el último del 2026-08-24 08:00 — 15 resueltos, 31 descartados, 6 detectados sin atender.
  **El pendiente (a) — archivar `mysql-weekly-report/` — está hecho de facto:** la tarea de Task Scheduler "Reporte Semanal MySQL" ya no ejecuta el Node viejo, apunta a `server-api/scripts/trigger-schema-health.ps1` (última corrida 2026-08-24, resultado 0; próxima 08-31), y el último output de `mysql-weekly-report/` es del 2026-07-06: ocho semanas sin correr. Ya no hay nada que validar.
  **Deuda menor:** la carpeta `MyNode/mysql-weekly-report/` sigue en disco, huérfana. Borrarla es una decisión del usuario, no un pendiente de diseño — por eso no vuelve al backlog.
