...
| 🧪 53 | **Stock/GainsCapture** | `Agente_GainsCapture` — espíritu especulativo (distinto a Preservation que es defensivo). Opera sobre `categoriaActivo='N'` (volátiles): venta parcial por niveles de ROI (50%/100%/150%), validación Claude de técnicos (RSI_d/RSI_w/EMA) antes de cada nivel. Dos modos: `automatico` (LMT directo a IB + notif Telegram) y `autorizado` (propuesta Telegram /ok /no, timeout 30min → cancela sin ejecutar). **En prueba desde 2026-08-06.** **⛔ BLOQUEADO 2026-08-20 para AUTONOMO/PROD** — revisión crítica independiente ([[resultado-revision-opus-preservation-gainscapture]]) verificó en código: `maximiza_sell_lotes()` reparte por conteo de lotes no por cantidad, "25%/33%/100%" son etiquetas falsas y "100%" puede liquidar lotes que Claude nunca evaluó (`Class_customer.py:953-978`); el camino `vender_qty<=0` no deja log (`Class_DashBot.py:1016-1018`), por lo que semanas sin incidentes no prueban nada; sin techo de reglas fijas sobre la cantidad que decide Claude; sin gate cruzado contra los STOP de Preservation sobre el mismo símbolo; y sin interruptor propio, ver ítem #76 nota H9. Ver tabla completa de hallazgos y plan de trabajo en el resultado. **Avance 2026-08-22:** lotes ahora en orden ROI DESC (antes entregaba los más antiguos — agrava retroactivamente H1), `lotesGain()` descuenta lo ya vendido, tope duro `vender_qty <= position` con ganancia prorrateada, `min_roi` filtra lotes y `min_ganancia` filtra la clase vía `DataHub.gains_config()`. H9 revertido a semáforo único por decisión del usuario. **Sigue bloqueado por H1/H5.** Pendiente calibrar `min_ganancia` (200 sobre la clase deja solo escenario "100%"). | Alta |
...
| 🧪 76 | **Stock/Preservation** | **Preservation Fase 1 mejorada — configuración dinámica + validaciones de mercado** — Implementado 2026-08-03: (1) **gainInversion en $** — usar campo `sesion.gainInversion` (dólares absolutos, no %) como ROI mínimo requerido antes de vender. Stock=$100, Crypto=$20. Garantiza ganancia mínima despues de comisiones. (2) **Registro de cancelaciones** — cuando Preservation cancela una orden por ROI < roi_minimo, actualizar `order_trader` con status=CANCELED para que aparezca en lista de órdenes. (3) **Validación RSI semanal** — solo permitir venta si RSI_semanal >= 70 (overbought). (4) **Validación precio mínimo** — evitar venta forzada durante correcciones: Stock=$50, Crypto=$0.001. (5) **Reducción de logs** — comentar mensajes [CLAUDE] y [ATR-CAP] intermedios que no resultan en orden. **Estado:** En observación. Ahora ejecutando correctamente (2026-08-13 07:31): Preservation(Stock) roi_min=10% prot=40%, Preservation(Crypto) roi_min=18% prot=40%. Validar 1-2 semanas en producción. **⛔ BLOQUEADO 2026-08-20 para AUTONOMO/PROD** — revisión crítica independiente ([[resultado-revision-opus-preservation-gainscapture]]) verificó en código: el ratchet del stop se rompe por round-trip del estado — `stop_final` se cachea contra `stop_max` pero se persiste igual aunque no se haya tocado la orden en IB, JSON y broker pueden divergir sin señal (`Class_AgentManager.py:1086-1088,1247-1249`); cancel+send sin verificación ni try por símbolo, una excepción aborta el resto de posiciones del vehículo por 12h (`:787-794,1116-1119`); Preservation sobre Crypto nunca envía órdenes reales pese al validador H4 documentado arriba, queda en `[DRY-RUN]` permanente (`:1102`); `PRECIO_MINIMO=50.0` hardcodeado no usa el `$0.001` de Crypto documentado en el punto (4) de este mismo ítem (`:1030-1035`); fallback silencioso `sma_base=last` cuando falla SMA20 queda fijado por el ratchet aunque la SMA vuelva (`:1042-1047`). Ver tabla completa de hallazgos y plan de trabajo en el resultado. **Decisión H6 (2026-08-21):** Crypto sacado explícitamente del loop de `Agente_ManagerPreservation` (`Class_AgentManager.py:782`, `for vehiculo in ("Stock",):`) — antes corría siempre en `[DRY-RUN]` sin protección real; se prefiere no simular en silencio. **Prueba en curso queda acotada a Stock únicamente.** Retomar Crypto cuando Stock esté sólido. GainsCapture (#53) sí cubre Stock+Crypto — no confundir alcance entre los dos agentes. | **Alta** |
...
| 83 | **Venta/Umbrales** | **Calibrar `min_ganancia` en Oportunidades y GainsCapture — es el umbral que realmente decide** — Medido contra `booktrading` el 2026-08-25 (filas `codigo='C'` agrupadas por símbolo+día = orden; ver `AppTest/run_booktrading_roi.py` y la sección 3.5 de [[design-autonomo]]). **Hallazgo:** el filtro que corta no es `min_roi` sino `min_ganancia`. En **Crypto** 22 de 31 órdenes históricas superaron el 30% de ROI y **una sola** superó los 300 USD — con los valores de hoy GainsCapture habría disparado una vez en dos años y medio (ganancia p50 = 29 USD, p75 = 71 USD, máxima 518 USD). En **Stock** 103 de 276 órdenes superaron el 20% de ROI y solo 15 superaron los 200 USD (ganancia p50 = 37 USD, p75 = 106 USD, máxima 389 USD) — mismo síntoma que ya estaba anotado en el ítem #53. **Lo que NO hay que tocar:** la amplitud del escalón de ROI (×2.2 Stock, ×1.5 Crypto) quedó medida y se deja como está — la franja solo-Oportunidades contiene 56 órdenes en Stock y 3 en Crypto, moverla mueve unidades. **Cautela al calibrar:** las órdenes históricas son ventas que ejecutó el usuario, no propuestas de los módulos; dicen el tamaño típico de la operación (que es lo relevante para un umbral en dólares) pero no si la venta fue buena, y en Crypto el tamaño de posición viene creciendo, así que la muestra vieja pesa hacia abajo. **Decisión pendiente del usuario** — no se toca un umbral en producción desde un análisis. | Media |
