...
| 🧪 76 | **Stock/Preservation** | **Preservation Fase 1 mejorada — configuración dinámica + validaciones de mercado** — Implementado 2026-08-03: (1) **gainInversion en $** — usar campo `sesion.gainInversion` (dólares absolutos, no %) como ROI mínimo requerido antes de vender. Stock=$100, Crypto=$20. Garantiza ganancia mínima despues de comisiones. (2) **Registro de cancelaciones** — cuando Preservation cancela una orden por ROI < roi_minimo, actualizar `order_trader` con status=CANCELED para que aparezca en lista de órdenes. (3) **Validación RSI semanal** — solo permitir venta si RSI_semanal >= 70 (overbought). (4) **Validación precio mínimo** — evitar venta forzada durante correcciones: Stock=$50, Crypto=$0.001. (5) **Reducción de logs** — comentar mensajes [CLAUDE] y [ATR-CAP] intermedios que no resultan en orden. **Estado:** En observación. Testeado con BP (cancelación registrada, remoción por price floor), cambios implementados en `Class_DashBot.py` (líneas 1287-1482) pero NO committeados. Pendiente: validar 1-2 semanas en producción antes de hacer commit. | **Alta** |
| 🧪 77 | **Stock/Diversificación UI** | **Gráficos de rentabilidad (dividendos + performance acumulado) — normalización de parámetros vehiculo** — Implementado sesión 2026-08-02 (parcial). **Bug raíz:** parámetros vehiculo inconsistentes en múltiples módulos — algunos usaban "hist"/"download" (string hardcodeado inválido), otros "Stock"/"Crypto". Gráficos no se mostraban para Stock/Crypto; solo para FCI (que usa vehiculo="ARS"). **Cambios aplicados:** (1) `dividends_rendimiento.py:30` — cambiar "hist" → "Stock". (2) `Modulos_Comunes.py:573` — cambiar typo "donwload" → "download". (3) `Class_customer.py:4821/4823/4754` — mapear vehiculos correctamente a "Stock" y "Crypto". (4) `ts_yfinance_symbol()` — validación _vehiculo agregada pero luego eliminada (causaba cache duplicates — approached rechazado). (5) **Validación yfinance:** `yf.Ticker().history()` SÍ retorna Dividends (confirmado). `yf.download()` NO retorna Dividends (comportamiento expected). **Estado:** En observación — múltiples intentos/reversiones durante debug (cambios implementados pero NO committeados por "no confirmes tengo otro ajuste"). Datos en `diaria_cnv` verificados (412 registros presentes). **Pendiente:** (a) validar que los 3 gráficos se renderizan correctamente en próxima sesión (rentabilidad, rentabilidad dividendos, performance acumulado) para Stock/Crypto/FCI; (b) si hay errores residuales, diagnosticar con `get_yfinance()` access pattern. Cambios críticos en `Class_AnalisisDiversificacion` (window_estrategia) y métodos de acceso a datos. | Media |
