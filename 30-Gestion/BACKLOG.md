...
| 🧪 77 | **Stock/Diversificación UI** | **Gráficos de rentabilidad (dividendos + performance acumulado) — filtrar datos NaN de yfinance** — **Sesión 2026-08-04:** Ubicado bug real en `draw_rentabilidad()` (Class_DataFrame.py:3034-3035). Cuando yfinance devuelve último Close=NaN (ej: PLUG), `float(NaN)` causa fallo silencioso. **Fix aplicado:** (commit 3ff3892) Filtrar `datos_valid = datos[datos["Close"].notna()]` antes de usar price_now. Valida que datos_valid no esté vacío, sino muestra "Sin datos válidos". **Resultado parcial:** ✅ Stock (PLUG) — gráfico rentabilidad funciona. ❌ FCI (FBA HORIZONTE) — aún no muestra gráfico de rentabilidad. Investigando: `get_yf_CNV()` retorna datos vacíos para FBA HORIZONTE (verificar si hay registros en `diaria_cnv` para ese símbolo). **Cambios previos NO necesarios** (revertidos parcialmente): correcciones en `dividends_rendimiento.py`, `Modulos_Comunes.py`, `Class_customer.py` — el problema real era filtrado de NaN, no parámetros vehiculo. **Pendiente:** (a) verificar datos de FCI en BD; (b) si hay datos, debuggear por qué `get_yf_CNV()` no los retorna; (c) validar que performance acumulado también se muestre. | Media |
