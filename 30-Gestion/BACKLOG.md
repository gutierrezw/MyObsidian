...
| 🧪 78 | **Infraestructura/Performance** | **Desface performa_inversion Crypto (2026-08-05)** — `p_vehiculo` anomalía: 2026-08-05 registra **4955.14** vs valores típicos 0.00-0.80 (500x mayor). Root cause: cambio `get_yfinance` ayer (2026-08-07) normalizó cantidad de datos descargados a 1800 (antes heterogéneo). **Fix aplicado:** `Agente_PerformaValidator` ahora valida **Stock + Crypto** (antes solo Stock, commit 10c3f03 2026-08-08). Agente ejecuta @1h — detectará anomalía, marcará para purga, ejecutará `validate_performa()` Crypto. **Estado:** En observación — esperando ejecución agente en próxima corrida (máx 1h). Si persiste después de próxima ejecución → ejecutar `run_solo_performa.py` manual. Datos: 1771 registros OK (2021-10-01→2026-08-07), solo 1 día desaceado. | Media |
| ✅ 77 | **Stock/Diversificación UI** | **Gráficos de rentabilidad — CERRADO** — Stock ✅ Crypto ✅ funcionan. Fix: `crea_dataframe_index()` convierte BTCUSDT → BTC-USD; `draw_rentabilidad()` filtra NaN. FCI investigación futura. | Media |
