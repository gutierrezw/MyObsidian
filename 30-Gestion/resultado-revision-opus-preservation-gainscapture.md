...
| H2 | `if vender_qty <= 0: continue` sin log — cada vez que H1 da 0, la decisión de Claude se evapora sin rastro. Las "semanas sin incidentes" no prueban prudencia | 🔴 Bloqueante | `Class_DashBot.py:1016-1018` — confirmado exacto, sin log en esa rama | **Antes de tocar código:** correr query sobre `symbol_decision_history` (`agente='GainsCapture'`, `tag='CLAUDE'`, filtrando decisiones tipo vender) vs órdenes efectivamente enviadas, para saber cuánto del historial "limpio" fue prudencia real y cuánto fue este bug. Luego loguear explícitamente el caso `vender_qty<=0` |
| H3 | Ratchet del stop se rompe por el round-trip del estado: `stop_final` se cachea contra `stop_max` (basado en `last-ATR` del ciclo), pero se persiste igual aunque no se haya tocado la orden — el JSON puede quedar por debajo del stop real en IB | 🔴 Bloqueante | `Class_AgentManager.py:1086-1088` (cap) + `:1247-1249` (persistencia incondicional del valor capado) — confirmado exacto | No persistir `stop_actual` capado si la orden no se modificó ese ciclo. Reconciliar `preservation_state.json` contra las órdenes vivas de IB al inicio de cada ciclo (usar el mismo mecanismo que ya existe para recuperar `order_id` vía `get_preservation_stops()`, línea 1125) |
| H4 | Cancel+send sin transacción ni verificación del retorno del cancel — gap sin protección si el send falla tras un cancel exitoso, y una excepción en un símbolo aborta el resto del vehículo (try/except está a nivel de vehículo, no de símbolo) | 🔴 Bloqueante | `Class_AgentManager.py:1116-1119` (cancel+send sin try propio) + `:787-794` (try/except envuelve el `for vehiculo`, no hay uno por símbolo dentro de `_preservation_run_vehiculo`) — confirmado exacto | Envolver el cuerpo por símbolo dentro de `_preservation_run_vehiculo` en su propio try/except (no dejar que una excepción aborte las posiciones restantes del vehículo). Verificar el retorno de `preservation_cancel_order` antes del `send`; si el cancel fallara silenciosamente, no perder el `order_id_prev` en el estado |
| H5 | STOP (Preservation) + LMT SELL (GainsCapture) pueden coexistir sobre el mismo símbolo sin vínculo OCA ni consulta cruzada de órdenes vivas — con escenario "100%" + Preservation 33%, hasta 133% de las acciones en ganancia comprometidas simultáneamente | 🔴 Bloqueante | Consecuencia directa de H1 (qty no acotada) + estados en dos JSON independientes (`preservation_state.json`, `gains_capture_state.json`) sin lock ni reconciliación — no requiere grep adicional, ya señalado como pregunta abierta en `design-gains-capture.md` | Gate cruzado antes de enviar cualquier orden de venta: consultar órdenes SELL vivas del otro agente sobre ese símbolo y verificar `qty_propuesta + qty_comprometida <= position`. Depende de resolver H1 primero (sin qty acotada, el gate no tiene contra qué comparar) |
| H6 | Preservation sobre Crypto nunca envía órdenes reales — calcula todo, arma la trama, loguea `[DRY-RUN]` y nunca manda nada | 🟠 Advertencia | `Class_AgentManager.py:1102` — `is_live = not self._preservation_dry_run and vehiculo == "Stock"` confirmado exacto | Decidir explícitamente: activar Crypto en Preservation, o sacar `"Crypto"` del loop de `Agente_ManagerPreservation` y documentar que Crypto no tiene protección de stops hoy — no dejarlo como "silencio con prefijo DRY-RUN" |
...
**No activar `AUTONOMO`/producción hasta 1-6.** Con eso resuelto y una semana de OBSERVACION
donde el log sí registre los no-ops (H2 corregido), la conversación sobre PROD tiene sentido.
