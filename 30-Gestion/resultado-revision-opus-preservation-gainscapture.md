...
|---|----------|-----------|----------------|----------|
| H1 | `maximiza_sell_lotes()` reparte por **conteo de lotes**, no por cantidad de acciones | 🟢 Reencuadrado (no bloqueante) | `Class_customer.py:953-978` — confirmado exacto: `cant_lote/c_sell <= threshold`, opera sobre lotes completos | ✅ **Reencuadrado y resuelto 2026-08-21** — confirmado con el usuario (dueño del diseño original) que "vender 25%/33%/100% de los lotes en ganancia" siempre fue por CONTEO de lotes, no de acciones: nunca fue un bug de cálculo. La consecuencia (25%/33% inalcanzables con pocos lotes) es matemática, no un error de código. Fix aplicado: `Class_DashBot.py` ahora calcula `escenarios_disponibles` según `len(list_gain)` (25% requiere ≥4 lotes, 33% requiere ≥3) y solo ofrece a Claude los escenarios matemáticamente alcanzables — antes se le ofrecían los 3 siempre y H2 solo detectaba el resultado en 0 después del hecho. Validado contra cartera real: 14/26 símbolos con 1-2 lotes ahora reciben solo `["100%"]`, 3 símbolos con 3 lotes reciben `["33%","100%"]`. Desbloquea #53, #76 pt.3/pt.6 (quedan pendientes por otras razones, no por H1) |
